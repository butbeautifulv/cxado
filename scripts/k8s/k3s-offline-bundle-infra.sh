#!/usr/bin/env bash
# Import third-party infra images into k3s containerd (nats, neo4j, obs base, toolbox).
#
# App images (egregore, veil-api/mcp): use Nexus Kaniko loops — see docs/deploy/nexus-*-loop.md
#
# Usage:
#   CXADO_OFFLINE_TAG=offline-YYYYMMDD ./scripts/k8s/k3s-offline-bundle-infra.sh
#
# Legacy name: k3s-offline-bundle-min.sh (removed — use this script).
set -euo pipefail
...
OUT_TAR="${CXADO_OFFLINE_TAR:-/tmp/cxado_offline_infra_${TAG}.tar}"

log() { printf '[k3s-offline-infra] %s\n' "$*"; }
...
transfer_and_import() {
  ...
  if [[ -n "${SUDO_PW}" ]]; then
    ...
    ssh -p "${SSH_PORT}" "${SSH_HOST}" "printf '%s\n' '${SUDO_PW}' | sudo -S -p '' k3s ctr images ls | grep -E 'neo4j|nats|redpanda' || true"
  else
    ssh -p "${SSH_PORT}" "${SSH_HOST}" "sudo k3s ctr images import '${remote_tar}'"
    ssh -p "${SSH_PORT}" "${SSH_HOST}" "sudo k3s ctr images ls | grep -E 'neo4j|nats|redpanda' || true"
  fi
}

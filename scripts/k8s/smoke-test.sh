...
curl_pod() {
  ...
  local port="$3"
  local path="$4"
  kubectl run "curl-$(date +%s)" --rm -i --restart=Never -n "$ns" --image=curlimages/curl:8.5.0 -- \
...
try_curl "veil-api" curl_pod veil veil-api 8090 /health
try_curl "veil-mcp" curl_pod veil veil-mcp 8091 /health

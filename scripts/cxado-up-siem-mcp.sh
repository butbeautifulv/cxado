#!/usr/bin/env bash
# Start maxpatrol-siem-mcp HTTP server on :8094 (host process, not Docker).
set -euo pipefail
...
log() { printf '[cxado-up-siem-mcp] %s\n' "$*"; }
...
if [[ ! -f "$SIEM_DIR/.env" ]]; then
  log "WARN: $SIEM_DIR/.env missing — copy from .env.example and set SIEM_* credentials"
fi
...
cd "$SIEM_DIR"
MCP_PORT="$PORT" nohup uv run maxpatrol-siem-mcp serve >"$LOG_FILE" 2>&1 &
echo $! >"$PID_FILE"
...
for i in $(seq 1 30); do
  if curl -sf -m 3 "http://localhost:${PORT}/health" >/dev/null 2>&1; then
    ...
    echo "  SIEM_MCP_ENABLED=true"
    echo "  SIEM_MCP_URL=http://localhost:${PORT}/mcp"
    exit 0
  fi
  sleep 1
done

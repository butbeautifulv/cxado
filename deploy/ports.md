...
Default profile `cxado-default`: Veil graph + egregore infra + observability. Egregore app runs on the host (`make -C projects/egregore dev`).

| Port | Service | Stack | Notes |
|------|---------|-------|-------|
| 3000 | egregore Operator UI | host | Next.js dev server |
| 3001 | Langfuse UI | optional | `cxado-langfuse` profile only |
| 3002 | Grafana | observability | admin/admin (dev) |
...
| 8080 | egregore API | host | `/metrics` for Prometheus |

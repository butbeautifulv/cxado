...
} from "@cxado/gui/charts/chart-card-layout"
import type { ChartSize } from "@cxado/gui/charts/chart-category-viewport"
import { cn } from "@cxado/gui/utils"
import type { ChartFilterScope, StatusDistribution } from "@cxado/gui/lib/charts/types"
...
export function buildFullDistribution(
  statusDistribution: StatusDistribution[],
  statusOrder: readonly string[],
  colors: readonly string[] = DEFAULT_CHART_COLORS
): StatusDistribution[] {
  ...
  return statusOrder.map((status, index) => ({
    ...
    fill: fillByStatus[status] ?? colors[index % colors.length] ?? "var(--chart-1)",
  }))
}

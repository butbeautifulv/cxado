...
import { Field, FieldGroup, FieldLabel } from "@cxado/gui/ui/field"
import { Textarea } from "@cxado/gui/ui/textarea"
import { notify } from "@cxado/gui/lib/ui/notify"
...
export function CommentaryAttachmentsField({
  ...
  const processFiles = useCallback(
    async (files: File[]) => {
      ...
      const remaining = maxAttachments - uploads.length
      if (remaining <= 0) {
        notify.error(`Максимум ${maxAttachments} изображений`)
        return
      }
    ...
    [disabled, presignUrl, uploads, syncAttachmentIds, maxAttachments]
  )
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          {uploads.map((item) => (
            <div
              key={item.id}
              className="group relative size-20 overflow-hidden rounded-gui-muted border"
            >
